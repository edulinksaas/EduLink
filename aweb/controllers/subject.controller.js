...
export const createSubject = async (req, res, next) => {
  try {
    ...
    // academy_id가 실제로 존재하는지 검증
    console.log('🔍 학원 존재 여부 확인 중 - academy_id:', academy_id);
    const academy = await Academy.findById(academy_id);
    
    if (!academy) {
      console.error('❌ 학원을 찾을 수 없습니다. academy_id:', academy_id);
      ...
      try {
        ...
        if (supabaseUrl && supabaseKey) {
          ...
          const { data: allAcademies } = await adminSupabase
            .from('academies')
            .select('id, name, code')
            .limit(10);
          
          if (allAcademies && allAcademies.length > 0) {
            console.log('📋 현재 데이터베이스에 있는 학원 목록:', allAcademies.map(a => ({ id: a.id, name: a.name, code: a.code })));
          } else {
            console.warn('⚠️ 데이터베이스에 학원이 없습니다.');
          }
        }
      } catch (debugError) {
        console.warn('⚠️ 학원 목록 조회 중 에러:', debugError.message);
      }
      
      return res.status(404).json({ 
        error: 'Academy not found', 
        message: `학원 ID '${academy_id}'에 해당하는 학원을 찾을 수 없습니다. 유효한 학원 ID를 사용해주세요.`,
        academy_id 
      });
    }
    
    console.log('✅ 학원 확인됨:', academy.name, academy.id);
