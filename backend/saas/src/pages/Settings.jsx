...
const Settings = () => {
  ...
  const [isTimetableEditMode, setIsTimetableEditMode] = useState(true); // 시간표/강의실 편집 모드
      ...
      if (userData?.academy_id) {
        try {
          ...
          if (academy) {
            ...
            // localStorage의 academy_name도 업데이트 (다음 로그인 시 일관성 유지)
            if (userData && academy.name !== userData.academy_name) {
              console.log('localStorage academy_name 업데이트:', userData.academy_name, '->', academy.name);
              userData.academy_name = academy.name;
              localStorage.setItem('user', JSON.stringify(userData));
            }
            return;
          }
        } catch (apiError) {
          console.warn('학원 정보 API 로드 실패, localStorage 사용:', apiError);
          // API 실패 시 localStorage 값 사용
          if (userData?.academy_name) {
            setAcademyName(userData.academy_name);
            setAcademyCode(userData.academy_code || '');
        ...
        <div className="academy-info-section">
          ...
          <div className="form-group">
            ...
            <p className="code-description">
              학원 코드는 학생 등록 시 사용되는 고유 코드입니다. 저장 시 자동으로 생성됩니다.
            </p>
          </div>
            ...
            <div className="form-group color-group">
              ...
              <div className="color-chips-wrapper">
                {colorOptions.map((color) => (
                  <div
                    key={color.name}
                    className={`color-chip ${subjectColor === color.value ? 'selected' : ''}`}
